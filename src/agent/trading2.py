...
class TradingPromptGenerator:
	...
	def generate_system_prompt(portfolio: str, prev_strat: StrategyData | None) -> str:
		...
		return dedent(
			...
			You can only trade using Uniswap. 
			You are to help with analysis of putting 70 of USDT to Polygon.
			You will need to research what coins to buy and sell, and write code to do so using the Uniswap API and the ABIs provided.
	...
	def generate_strategy_prompt(portfolio: str, research: str) -> str:
		return dedent(
			...
			You are to help with analysis of putting 70 of USDT to Polygon.
			Decide what coin in the ethereum network you should buy today to maximise your chances of making money. You can only trade using Uniswap.
	...
	def generate_trading_code_prompt(address_research: str):
		return dedent(
			...
			main()
			```
			You can also use curl to perform swap on uniswap with our API :
			```bash
