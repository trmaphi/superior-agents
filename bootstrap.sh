...
main() {
    ...
    check_python_version
    command -v docker >/dev/null 2>&1 || { echo >&2 "Error: Docker required but not found"; exit 1; }
    command -v docker compose >/dev/null 2>&1 || { echo >&2 "Error: Docker Compose required but not found"; exit 1; }
