...
class ActuatorSubsystem:
    tool = ToolRegistry(namespace="actuator")
...
class Agent:
    def __init__(self):
        self.actuator = ActuatorSubsystem()
        
    def execute_tool(self, full_name: str, *args, **kwargs):
        subsystem, name = full_name.split(".", 1)
        instance = getattr(self, subsystem)
        return instance.tool.execute(instance, name, *args, **kwargs)

# Usage
agent = Agent()
agent.execute_tool("actuator.move", 1.5, 3.2)  # Prints "Moving to (1.5, 3.2)"