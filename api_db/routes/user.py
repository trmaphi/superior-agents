...
def create_user(_x_api_key: X_API_KEY_DEPS, request: Request, params: intf_u.UserParams):
    req_data = params.__dict__
    _, users = db_u.get_all_users_db(intf_u.RESULT_COLS,{"wallet_address":params.wallet_address},{})
    if len(users) > 0:
        return {"status":"success", "data":users[0]}
    req_data['user_id'] = str(uuid.uuid4())
    db_u.insert_users_db(req_data)
    _, users = db_u.get_all_users_db(intf_u.RESULT_COLS,{"user_id":req_data['user_id']},{})
    return {'status':"success","msg":"user inserted", "data":users[0]}

@router.post('/api_v1/user/update')
def update_user(_x_api_key: X_API_KEY_DEPS, request: Request, params: intf_u.UserUpdateParams):
...
def get_user(_x_api_key: X_API_KEY_DEPS, request: Request, params: intf_u.UserUpdateParams):
    if params.user_id:
        count, results = db_u.get_all_users_db(intf_u.RESULT_COLS,{"user_id":params.user_id},{})
