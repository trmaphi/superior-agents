...
from src.helper import get_latest_notifications_by_source, delete_none
...
class APIDB:
	...
	def update_agent(self, agent_id: str, user_id: str = None, name: str = None, configuration: str = None) -> Dict[str, Any]:
		payload = {
			"agent_id": agent_id,
			"user_id": user_id,
			"name": name,
		...
		response = self._make_request("agent/update", payload, Dict[str, Any])
		
		if not response.success:
			raise ApiError(f"Failed to update agent: {response.error}")
		
		return response.data

	def create_agent_sessions(self, session_id: str, agent_id: str, status: str) -> Dict[str, Any]:
		payload = {
			"session_id": session_id,
			"agent_id": agent_id,
		...
		response = self._make_request("agent_sessions/create", payload, Dict[str, Any])
		
		if not response.success:
			raise ApiError(f"Failed to create agent: {response.error}")
