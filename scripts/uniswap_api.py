	...
	def swap_tokens(
		...
		try:
			...
			swap_txn = self.router_contract.functions.swapExactTokensForTokens(
				amount_in, min_amount_out, [token_in, token_out], self.address, deadline
			).build_transaction(
				{
					"from": self.address,
					"gas": 200000,
...
async def swap_tokens(
	...
	try:
		...
		return {"transaction_hash": "", "status": "failed", "reasons": str(e)}
...
async def get_balance(
	token_address: str, swapper: UniswapSwapper = Depends(get_swapper)
):
	try:
