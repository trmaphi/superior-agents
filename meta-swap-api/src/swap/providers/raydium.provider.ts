...
export class RaydiumSwapProvider implements ISwapProvider {
  ...
  async isSwapSupported(fromToken: TokenInfo, toToken: TokenInfo): Promise<boolean> {
    ...
    try {
      const response = await this.getSwapCompute(fromToken, toToken, new BigNumber(1));
  ...
  async getSwapQuote(params: SwapParams): Promise<SwapQuote> {
    ...
    return {
      ...
      expectedPrice: new BigNumber(response.data.outputAmount).dividedBy(response.data.inputAmount),
  ...
  async getUnsignedTransaction(params: SwapParams): Promise<SolUnsignedSwapTransaction> {
    ...
    const tx = this.txVersion === 'V0' 
      ? VersionedTransaction.deserialize(txBuf)
      : Transaction.from(txBuf);

    return {
      // @ts-expect-error
      instructions: 'instructions' in tx ? tx.instructions : tx.message.instructions,
    };
  }
