...
def insert_notifications_db(cursor, insert_dict):
...
@db_connection_decorator
def update_notifications_db(cursor, set_dict, where_dict):
...
@db_connection_decorator
def get_all_notifications_db(cursor, result_columns: list, where_conditions: dict, pagination) -> str:
    ...
    order_by_clause = f"ORDER BY sup_notifications.{pagination['sort_by']} ASC" if 'sort_by' in pagination else ""
