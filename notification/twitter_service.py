...
class Tweet(BaseModel):
    """
    Model representing a Twitter tweet with extracted information.

    Attributes:
        id (str): Tweet ID
        text (str): Tweet text content
        created_at (datetime): Tweet creation timestamp
        user_screen_name (str): Username of tweet author
        user_id (str): User ID of tweet author
        is_retweet (bool): Whether the tweet is a retweet
        is_quote (bool): Whether the tweet is a quote tweet
        mentioned_users (List[str]): List of mentioned usernames
        hashtags (List[str]): List of hashtags used
        urls (List[str]): List of URLs included
        media_urls (List[str]): List of media URLs (images, videos)
        quoted_tweet (Optional[Dict]): Information about quoted tweet if present
        retweeted_tweet (Optional[Dict]): Information about retweeted tweet if present
        reply_to_tweet_id (Optional[str]): ID of tweet being replied to
        reply_to_user_id (Optional[str]): ID of user being replied to
    """
    id: str
    text: str
    created_at: datetime
...
class TwitterService:
    def __init__(self, bot_username: str):
        ...
        Args:
            bot_username (str): The username of the Twitter bot
        """
        ...
        # Initialize API v1.1 with minimal retry settings
        self.api = tweepy.API(
            auth, 
        ...
        # Initialize Twitter API v2 client with minimal settings
    ...
    def _process_tweet(self, tweet) -> Tweet:
        ...
        Args:
            tweet: Raw tweet object from Twitter API

        Returns:
            Tweet: Processed tweet with extracted information including:
                - Basic tweet metadata (id, text, creation date)
                - User information
                - Tweet type (retweet, quote, reply)
                - Mentioned users, hashtags, URLs
                - Media content
                - Related tweet information (quoted/retweeted content)
        """
    ...
    def get_mentions(self, count: int = 10, since_id: Optional[str] = None) -> List[Tweet]:
        """
        Get recent mentions of the bot account using Twitter API v2.

        Args:
            count (int): Maximum number of mentions to retrieve (default: 10)
            since_id (Optional[str]): Only get mentions newer than this tweet ID

        Returns:
            List[Tweet]: List of processed tweets mentioning the bot
    ...
    def get_own_timeline(self, count: int = 10, since_id: Optional[str] = None) -> List[Tweet]:
        """
        Get recent tweets from bot's own timeline using Twitter API v2.

        Args:
            count (int): Maximum number of tweets to retrieve (default: 10)
            since_id (Optional[str]): Only get tweets newer than this tweet ID

        Returns:
            List[Tweet]: List of processed tweets from bot's timeline
    ...
    def extract_trading_signals(self, tweet: Tweet) -> Optional[Dict]:
        """
        Extract trading signals from a tweet's content.

        Args:
            tweet (Tweet): Tweet to analyze for trading signals

        Returns:
            Optional[Dict]: Dictionary containing extracted trading signals if found,
    ...
    def extract_market_events(self, tweet: Tweet) -> Optional[Dict]:
        """
        Extract market events from a tweet's content.

        Args:
            tweet (Tweet): Tweet to analyze for market events

        Returns:
            Optional[Dict]: Dictionary containing extracted market events if found,
                          None if no events detected
        """
        text = tweet.text.lower()
        events = {}
    ...
    def check_rate_limit(self):
        """
        Check current rate limit status for Twitter API endpoints.
        
        Logs the remaining requests and reset time for relevant endpoints.
        """
        try:
