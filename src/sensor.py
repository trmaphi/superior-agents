...
class AgentSensor:
	...
	def get_sample_of_recent_tweets_of_followers(self) -> List[TweetData]:
		...
		if err := tweets_result.err():
			logger.error(f"AgentSensor.get_sample_of_recent_tweets_of_followers, failed to get tweet samples, err: \n{err}")
			return MOCK_TWEETS
	...
	def get_count_of_followers(self) -> int:
		...
		if err := get_result.err():
			logger.error(f"AgentSensor.get_count_of_followers, failed to get count of followers, err: \n{err}")
			return 27
